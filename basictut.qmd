...
R will also follow the order of expressions, or how most of us know it: PEMDAS (Parentheses, exponents, multiplication, division, addition, and subtraction). This is especially important if you ever do a long formula.
...
Any expression you type into the console can be stored as a variable, which can be called later on. These variables can also be used in conjunction with different math expressions. It is important to note that when creating a variable, the variable can be used interchangeably with the number(s) stored inside of it.
...
::: callout-tip
**Try it out:** Create a variable called `name` and have this equal your name. After you have done so, type the variable `name` and press enter.
:::
...
You most likely got an error message. Here is why...

### **Data Types**

In R, there are different data types or classes that R will recognize. We will first focus on `Numeric` and `Character`. Take note of the `class()` function that identify how `R` evaluates this variable.
...
If you wanted to see what the 5^th^ number was in the `fib` vector, you would put a bracket `[]` next to the variable name. Inside of the brackets you would put the position you wanted to see.
